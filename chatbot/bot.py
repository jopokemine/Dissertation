...
# datafile = os.path.join("data", "formatted_lines_combined.txt")
...
def zeroPadding(li, fillvalue=PAD_token):
    return list(itertools.zip_longest(*li, fillvalue=fillvalue))


def binaryMatrix(li, value=PAD_token):
    m = []
...
def inputVar(line, voc):
    indexes_batch = [indexesFromSentence(voc, sentence) for sentence in line]
